...
void get_timer(Timer *timer, double *realtime, double *usertime, double *systime)
{
	...
	(*realtime) = (double) (real_now - timer->real_time) / CLOCKS_PER_SEC;
	(*usertime) = (double) (now.tms_utime - timer->user_time) / CLOCKS_PER_SEC;
	(*systime) = (double) (now.tms_stime - timer->system_time) / CLOCKS_PER_SEC;
}
